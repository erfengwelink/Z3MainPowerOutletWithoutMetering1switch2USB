...
//extern EmberAfStatus localManuTouchControl(uint8_t cmd, uint8_t ep, bool isInNWK);
                                                  ...
                                                  bool initiatedByLevelChange)
{
	...
	emberAfOnOffClusterPrintln("emberAfReadAttribute currentValue %d", currentValue);
	...
	emberAfOnOffClusterPrintln("Toggle endpoint %d on/off from %x to %x", endpoint, currentValue, newValue);
	bool isInNwk = emberAfNetworkState() == EMBER_JOINED_NETWORK ? IS_IN_NWK : NOT_IN_NWK;
	EmberAfStatus est = ControlComandHandle(command, endpoint, isInNwk, false);
	emberAfOnOffClusterPrintln(" localManuTouchControl -> EmberAfStatus: %d", est);
//	if (newValue) {
